...
public class DruidStatViewExtendHandler extends Handler {
    ...
    public DruidStatViewExtendHandler(String visitPath, IDruidStatViewAuth druidStatViewAuth) {
    ...
    public void handle(String target, HttpServletRequest request, HttpServletResponse response, boolean[] isHandled) {
        if (target.startsWith(visitPath)) {
            ...
            if (target.equals(visitPath) && !target.endsWith("/index.html")) {
                ...
                /* add by JFnice */
                HandlerKit.redirect(request.getContextPath() + target + "/index.html", request, response, isHandled);
                return;
            }
    ...
    class JFinalStatViewServlet extends StatViewServlet {
        ...
        public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            ...
            if (isRequireAuth() //
                    ...
                    && !("/loginout.html".equals(path) //
                    || path.startsWith("/css")//
                    || path.startsWith("/js") //
                    || path.startsWith("/img"))) {
                if (contextPath == null || contextPath.equals("") || contextPath.equals("/")) {
                    response.sendRedirect("/druid/loginout.html");
                } else {
                    if ("".equals(path)) {
                        response.sendRedirect("druid/loginout.html");
                    } else {
                        response.sendRedirect("loginout.html");
                    }
                }
                return;
            }
