...
public class JFniceBaseServiceOld<M extends Model<M>> implements JFniceBaseSer<M> {
	...
	public JFniceBaseServiceOld(String parentKey) {
		try {
	...
	public <T> M queryById(T id, String fields) {
		Kv kv = Kv.by("fields", fields)
				  ...
				  .set("id", id);
		SqlPara sqlPara = Db.getSqlPara("JFniceBaseOld.queryById", kv);
	...
	public <T> List<M> queryListByIds(T[] ids, String fields, String orders) {
		...
		Kv kv = Kv.by("fields", fields)
				  ...
				  .set("orders", orders);
		return dao.find(Db.getSqlPara("JFniceBaseOld.queryListByIds", kv));
	}
	...
	public List<M> queryList(String fields, Okv conditions, String orders) {
		Kv kv = Kv.by("fields", fields)
				  ...
				  .set("orders", orders);
		return dao.find(Db.getSqlPara("JFniceBaseOld.queryPageOrList", kv));
	}
	...
	public Page<M> queryPage(CondPara condPara) {
		condPara.set("tableName", getTableName())
				.set("primaryKey", getPrimaryKey());
		SqlPara sqlPara = Db.getSqlPara("JFniceBaseOld.queryPageOrList", condPara.toKv());
	...
	public <T> List<M> queryAllRelativeList(T id, String fields, boolean downFlag, boolean showAllStatusFlag) {
		...
		SqlPara sqlPara = Db.getSqlPara("JFniceBaseOld.queryAllRelativeList", kv);
	...
	public <T> boolean deleteById(T id, boolean isRealDelete) {
		...
		SqlPara sqlPara = Db.getSqlPara("JFniceBaseOld.deleteById", kv);
	...
	public <T> int toggleField(T id, String field) {
		Kv kv = Kv.by("tableName", getTableName())
				  .set("primaryKey", getPrimaryKey())
				  .set("field", field);
		SqlPara sqlPara = Db.getSqlPara("JFniceBaseOld.toggleField", kv);
	...
	public <K, V> int[] sort(Map<K, V> map) {
		...
		for ( Map.Entry<K, V> entry : map.entrySet() ) {
			...
			recordList.add(new Record().set(getPrimaryKey(), id).set("sequence", sort));
		}
		return Db.batchUpdate(getTableName(), recordList, recordList.size());
	}
	...
	public void buildTree(M topM, List<M> mList) {
		...
		for ( M m : mList ) {
			if ( m.get(getPrimaryKey()) == null || m.get(getParentKey()) == null || m.get("sequence") == null ) {
				throw new RuntimeException("字段参数至少应包含" + getPrimaryKey() + "、" + getParentKey() + "、sequence");
			}
			map.put(m.get(getPrimaryKey()), m);
		}
		...
		sortChildList(root.get("children"));
		topM.put("children", root.get("children"));
	}
	...
	public class ChildComparator implements Comparator<M> {
		public int compare(M m1, M m2) {
			if ( m1.getLong("sequence").equals(m2.getLong("sequence")) ) {
				if ( Number.class.isAssignableFrom(getPrimaryKeyType()) ) {
			...
			return m1.getLong("sequence") < m2.getLong("sequence") ? -1 : 1;
		}
	}
	...
	public boolean isUnique(M m, String... fields) {
		...
		Kv kv = Kv.by("tableName", getTableName())
				  ...
				  .set("fields", fields)
				  .set("m", m);
		SqlPara sqlPara = Db.getSqlPara("JFniceBaseOld.isUnique", kv);
	...
	public boolean hasChild(M m) {
		...
		SqlPara sqlPara = Db.getSqlPara("JFniceBaseOld.hasChild", kv);
		return null != dao.findFirst(sqlPara);
	}
